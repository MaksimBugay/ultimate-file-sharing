...
The page keeps one in-memory array of complete audio/video chunk pairs. Each entry has `audioBinary` and `videoBinary` blobs, `index`, `startMs`, `endMs`, and `durationMs`. The time range `[startMs, endMs)` follows the audio segment and uses milliseconds from the start of the local recording; `endMs` is captured when that segment is stopped, and `durationMs` is `endMs - startMs`. These are approximate capture times, not timestamps embedded in the saved media files. For future echo cancellation, match mic ranges against the actual local playback ranges of received audio, which may cross chunk boundaries. A continuous recorder runs alongside the standalone segment recorders so the saved files have valid, uninterrupted media containers. Segment boundaries can have small gaps because each standalone segment starts a fresh encoder; the saved continuous files avoid those gaps. The recording remains in memory until the tab closes.

While recording, **Save audio from 12–5s ago** becomes available when completed microphone chunks cover that seven-second interval. It selects those stored chunks by `startMs` and `endMs`, trims them to the requested range, and saves a mono PCM WAV file. Small gaps between standalone chunks become silence in the WAV. The clip contains microphone audio for a future echo-cancellation step; no echo cancellation is applied here.
